/*
 * manageRoles.js - main javascript resource for the APAF Application Manage Roles Page
...
const DATA_MANAGER_ID = 'roleManager';
...
initNewRecord = function(){
	...
	selectList.select(-1);
	let newRole = {};
	newRole.name = 'someRole';
	newRole.description = 'Some description';
	let form = npaUi.getComponent(EDIT_FORM_ID);
	form.setData(newRole);
...
deleteRecord = function(){
	...
	if(confirm(npaUi.getLocalizedString('@apaf.page.roles.delete.confirm',[currentRecord.name]))){
		let dataManager = npaUi.getComponent(DATA_MANAGER_ID);
		dataManager.delete(currentRecord).then(function(data){
			...
			flash('@apaf.page.roles.delete.flash');
		}).onError(function(errorMsg){
			if(errorMsg.httpStatus==404){
				showError('@apaf.error.http.not.found');
			}else
