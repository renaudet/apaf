/*
 * manageGroups.js - main javascript resource for the APAF Application Manage Groups Page
...
const DATA_MANAGER_ID = 'groupManager';
...
initNewRecord = function(){
	...
	selectList.select(-1);
	let newGroup = {};
	newGroup.name = 'someGroup';
	newGroup.description = 'Some description';
	newGroup.roles = [];
	let form = npaUi.getComponent(EDIT_FORM_ID);
	form.setData(newGroup);
...
deleteRecord = function(){
	...
	if(confirm(npaUi.getLocalizedString('@apaf.page.groups.delete.confirm',[currentRecord.name]))){
		let dataManager = npaUi.getComponent(DATA_MANAGER_ID);
		dataManager.delete(currentRecord).then(function(data){
			...
			flash('@apaf.page.groups.delete.flash');
		}).onError(function(errorMsg){
			if(errorMsg.httpStatus==404){
				showError('@apaf.error.http.not.found');
			}else
